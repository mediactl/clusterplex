...
init_plex_directories() {
    ...
    if [[ ! -f "$plex_dir/Preferences.xml" ]]; then
        local machine_id
        # Ours: upstream pipes this through `tr -d '-'`, which turns a perfectly
        # good UUID into 32 bare hex characters. Plex parses the identifier as a
        # UUID and dies on the first plug-in that reads it:
